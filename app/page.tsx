      ...
      <section className="dj-hero">
        <div className="dj-hero-copy">
          ...
          <div className="dj-hero-actions">
            <Link href="/cases" className="dj-btn dj-btn-light">Mulai Clinical Cases <span>→</span></Link>
            <Link href="/tools" className="dj-btn dj-btn-ghost">Explore Clinical Tools</Link>
          </div>
        ...
        <div className="dj-hero-art" aria-hidden="true">
          ...
          <div className="anatomy-ring"><span>♥</span></div>
        </div>
      </section>

      <section className="dj-search-wrap">
        <div className="dj-search"><span className="search-glyph">⌕</span><div><strong>What are you looking for?</strong><span>Search cases, diagnosis, drugs, guidelines, or clinical tools...</span></div><kbd>⌘ K</kbd></div>
      </section>

      <section className="dj-section">
        <div className="dj-section-head"><div><span className="dj-kicker">YOUR CLINICAL WORKSPACE</span><h2>Everything you need, in one place.</h2></div><span className="dj-section-note">Built for everyday practice</span></div>
        <div className="dj-module-grid">
          {modules.map((m) => <Link href={m.href} className={`dj-module ${m.tone}`} key={m.title}><div className="module-icon">{m.icon}</div><div className="module-copy"><span>{m.eyebrow}</span><h3>{m.title}</h3><p>{m.desc}</p></div><strong className="module-count">{m.count}</strong><span className="module-arrow">↗</span></Link>)}
        </div>
      </section>

      <section className="dj-feature-grid">
        <Link href="/cases" className="dj-case-feature">
          <div className="feature-label">CASE OF THE DAY <span>5 MIN READ</span></div>
          <div className="case-illustration"><div className="ecg-line">〰〰╱╲〰╱╲〰〰</div><div className="case-circle">♥</div></div>
          <div className="case-feature-copy"><span>EMERGENCY · CARDIOLOGY</span><h2>Acute Chest Pain</h2><p>45-year-old man datang dengan nyeri dada akut. Prioritaskan assessment, ECG, differential diagnosis, dan keputusan reperfusi.</p><div className="case-tags"><b>STEMI</b><b>ECG</b><b>Shock</b></div><strong className="feature-cta">Open case <span>→</span></strong></div>
        </Link>
        <div className="dj-tools-feature"><div className="dj-section-head compact"><div><span className="dj-kicker">QUICK TOOLS</span><h2>Useful when you need them.</h2></div><Link href="/tools">View all →</Link></div><div className="tool-list-premium">{tools.map((tool, i) => <Link href="/tools" key={tool}><span>{String(i + 1).padStart(2, '0')}</span><strong>{tool}</strong><i>→</i></Link>)}</div></div>
      </section>

      <section className="dj-topics">
        <div className="dj-section-head"><div><span className="dj-kicker">EXPLORE BY TOPIC</span><h2>Build clinical confidence.</h2></div><Link href="/library">Open Library →</Link></div>
        <div className="topic-pills">{topics.map((topic, i) => <Link href="/library" key={topic}><span>{String(i + 1).padStart(2, '0')}</span><strong>{topic}</strong><b>↗</b></Link>)}</div>
      </section>
