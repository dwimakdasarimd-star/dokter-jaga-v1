...
type Tool = 'bmi' | 'egfr' | 'crcl' | 'bsa' | 'gcs' | 'map' | 'shock' | 'anion' | 'correctedNa' | 'pedsFluid' | 'tpm'
...
export default function ToolsPage(){
  ...
  return <>
    ...
    <div className="section tools-page">
      ...
      <div className="tool-search"><span>⌕</span><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Cari BMI, eGFR, TPM, GCS, anion gap..."/></div>
      <div className="tool-layout">
        ...
        <main className="tool-workspace">
          ...
          {active==='tpm'&&<div className="tool-panel tpm-panel">
            <div className="tpm-intro"><div><span className="meta">IV FLOW RATE</span><h3>TPM Infus Calculator</h3><p>Masukkan volume cairan, durasi pemberian, dan drop factor sesuai kemasan infus set.</p></div><span className="tool-status">Real-time calculator</span></div>
            <div className="tool-fields">
              <Field label="Volume cairan" value={tpmVolume} onChange={v=>{setTpmVolume(v);setTpmCalculated(false)}} unit="mL" placeholder="500"/>
              <Field label="Waktu pemberian" value={tpmTime} onChange={v=>{setTpmTime(v);setTpmCalculated(false)}} unit="jam" placeholder="4"/>
              <label className="tool-field"><span>Drop factor infus</span><select value={dropFactor} onChange={e=>{setDropFactor(e.target.value);setTpmCalculated(false)}}><option value="10">Macrodrip — 10 gtt/mL</option><option value="15">Macrodrip — 15 gtt/mL</option><option value="20">Macrodrip — 20 gtt/mL</option><option value="60">Microdrip — 60 gtt/mL</option></select></label>
            </div>
            <div className="tpm-actions"><button type="button" className="primary-tool-btn" onClick={()=>setTpmCalculated(true)}>Hitung TPM</button><button type="button" className="secondary-tool-btn" onClick={clearTPM}>Reset</button></div>
            {tpmCalculated&&<div className="result-grid tpm-results"><Result label="TPM / tetes per menit" value={Number.isFinite(tpm.gtt)?`${Math.round(tpm.gtt)} tetes/menit`:'Input belum lengkap'} note={Number.isFinite(tpm.gtt)?'Dibulatkan ke tetes/menit terdekat.':undefined}/><Result label="Kecepatan cairan" value={Number.isFinite(tpm.mlhr)?`${tpm.mlhr.toFixed(1)} mL/jam`:'—'}/></div>}
            <div className="tpm-formula"><strong>Rumus</strong><code>TPM = volume (mL) × drop factor (gtt/mL) ÷ waktu (menit)</code><span>Waktu dalam kalkulator dimasukkan dalam jam → otomatis × 60 untuk mendapatkan menit.</span></div>
            <div className="tpm-example"><strong>Contoh:</strong> NaCl 0,9% 500 mL selama 4 jam dengan macrodrip 20 gtt/mL → <b>42 tetes/menit</b> dan <b>125 mL/jam</b>.</div>
            <p className="tool-note">⚠️ Selalu cek drop factor yang tercetak pada infus set (misalnya 10, 15, 20, atau 60 gtt/mL). Verifikasi hasil dan konteks klinis sebelum terapi diberikan.</p>
          </div>}
